...
#But for embryos sequenced on only one lane (6X target depth samples), there was no association of coverage and what lane the embryo was sequenced on. 
...
Anova(model, type="III") #not unexpected - didn't bother to remove potential outliers from this analysis since supposedly ANOVA is robust against deviances from normality.  
...
#I just read in a different file and changed the labels and such to plot the others. 
...
#to be conservative, we are going to exclude the three in hupehensis that deviate slightly from the main cluster. Those may be selfs, or have a slightly higher error rate since they were sequenced at slightly lower depths.

#less conservative - assuming maybe those 3 aren't selfs and are actually clonal.
...
#more conservative; excluding the three that might be selfs.
...
######################## combine the "true clonal embryos" datasets from the two moms and figure out a stricter mathematic range... using the IQR 1.5 rule. 
library(stats)
...
# we also need to consider what the spike-ins percentages look like, to get a better picture of the error that could be associated with library prep differences. 

################################### Plot the results for the 4 6X control genotypes #####################################

#set the order of moms in the data frame because I'm picky about colors. 
...
#set a filter on which embryos to consider. Namely - drop ones that have too few sites in the het or hom category.
...
boxplot(all_spikeins$het_match_percentage) #LOL those tetraploid platycarpas... what the?
all_spikeins$embryo[which(all_spikeins$total_match_percentage < 90)] #what's wrong with these little monsters wtf

# Take out the problem children for now >.<, and filter spikeins that don't have enough sites to evaluate.
...
## what are some metrics of similarities if we ignore spikeins with too low coverage and the tetraploid platycarpa issue?
...
# based on 6X controls, spikeins, and simulated embryos...
# we will go with 92.5% for the het percentage, and
# 95% should be a suitable cutoff for hom.

#The tetraploid platycarpas are a bit of a different story though. Let's look at the metrics for their spikeins.

#We'll have to adjust the quadrants for the tetraploid Mplatycarpas.
...
#might be able to answer this question someday. Putting a mental pin on this one.  
...
##following the convention that the range may actually be a little lower for lower coverage embryos, we'll do 85 het, 82 hom
...
##combine the two big dataframes for plotting - but drop the controls. That will be a separate figure with the flow cytometry spectra example.
ALL <- rbind(all_results_16moms_no_sims, all_results_3Xembryos_true_no_sims) %>%
  dplyr::filter(!mother %in% c("Mspectabilis_588917", "Masiatica_594099", "Mhupehensis_633818", "Msargentii_589400"))

#order levels for plotting. There's gotta be a better way of doing this lollllll....
...
#I'll add seed year column to both before combining for extra safety in combining the rows.  
...
#really didn't have to have d or e included in that [ ] I guess. Only ever did up to 3 spikeins.
...
SUPPTABLE2 <- SUPPTABLE2 %>%
  mutate(
    quadrant = case_when(
      ...
      # Special genotype samples (custom thresholds)
      ...
      # samples with normal spikein values (standard thresholds)
...
########## checking association between depth and variant calling error rate -- reported in the methods.  
